import { Component, ChangeDetectorRef, Input, OnInit, Sanitizer } from '@angular/core';
...
export class ImageComponent implements OnInit {
    @Input() width: number = 1;  // style show width
    @Input() height: number = 1; 
    @Input() scaleWidth: number = 1;   // save image width
    @Input() scaleHeight: number = 1; 
    @Input() max: number = 1; 

    @Input() imgCircle: number = 0;
    @Input() imgCenter: number = 0;
    @Input() rmColor: string = 'ff0000'; 
    @Input() onFinish: Function = null; 

    private _images: any[] = [];
    @Input() set images(values: string[]) {
        for(var i = 0 ; i < values.length ; i ++) {
            if(values[i]) this._images.push(this.sanitizer.bypassSecurityTrustResourceUrl(values[i]));
        }
    }
    get images(): string[] {
        return this._images;
